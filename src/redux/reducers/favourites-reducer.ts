...
import { AppStateType, BaseThunkType, DispatchThunkType, InferActionsTypes } from "../store"
...
export const favouritesReducer = (state = initialState, action: ActionsType): InitialStateType => {
    switch (action.type) {
        ...
        case 'FAVOURITES-RD/DELETE-ONE-ITEM':
            return {
                ...state,
                favourites: state.favourites.filter(item => item.image_id !== action.id)
            }
        default:
            return state
    }
...
export const deleteFromFavourites = (favitemID: string | number, id: string, url: string) => async (dispatch: DispatchThunkType) => {
    try {
        dispatch(actions.setinToFromFavouritesProccess(true, id))
        const data = await favouritesAPI.deleteFromFavourites(favitemID)
